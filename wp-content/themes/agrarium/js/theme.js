...
function sticky_menu_active(once) {
  var stickyHeaders = jQuery(
    '[class~="sticky-header-on"]:not(.header-type-3):not(.mobile-header-type-3)'
  );
...
function sticky_menu_header_type_3_active(once) {
  var stickyHeaders = jQuery(
    ".header-type-3.sticky-header-on, .mobile-header-type-3.sticky-header-on"
  );
  if (stickyHeaders.length) {
    stickyHeaders.each(function () {
      let obj = jQuery(this);
...
function elements_slider_init(elements) {
  var $elements = elements
    ? jQuery(elements)
    : jQuery(".elementor-element .owl-carousel");
...
function custom_video_play_button() {
  jQuery(".mejs-overlay-button").each(function () {
    jQuery(this).html(
      '<svg viewbox="0 0 11 17"><path fill-rule="evenodd" fill="rgb(31, 30, 23)" d="M0.963,15.764 C0.963,15.26 0.963,2.62 0.963,1.552 C0.963,0.893 1.406,0.400 1.844,0.787 C2.194,1.97 9.205,7.344 9.803,7.879 C10.196,8.229 10.189,9.66 9.803,9.419 C9.372,9.810 2.335,16.90 1.826,16.526 C1.452,16.845 0.963,16.492 0.963,15.764 Z"/></svg>'
    );
  });
}
  ...
  if (jQuery(".agrarium-productions-listing-widget.offset").length > 0) {
    jQuery(".agrarium-productions-listing-widget.offset").each(function () {
      ...
      if ($container.length && window.innerWidth >= 768) {
        ...
        $container.css({
          marginRight: 0 - marginLeft - payload - scrollbarWidth,
        });
      } else {
  ...
  if (jQuery(window).width() >= 992) {
    if (jQuery(el).hasClass("scroll-motion-direction-up")) {
      var from_top =
        ...
        (widget_container[0].getBoundingClientRect().top +
          widget_container.height() / 2);
      if (from_top >= range) {
        from_top = range;
...
jQuery(document).ready(function () {
  ...
  if (jQuery(window).width() >= 1025) {
    jQuery(".agrarium_content_slider_widget").each(function () {
      ...
      setTimeout(function () {
        slider
          .find(".agrarium_content_slide")
          .first()
          .removeClass("first-loaded");
      }, 30);
    });
  }
  ...
  setTimeout(function () {
    ...
    elements_slider_init(
      ":not(.production-slider-listing):not(.portfolio-slider-wrapper)"
    );
  }, 100);
  ...
  if (
    jQuery("#inner-pages-carousel").length > 0 &&
    ...
    $(".elementor-widget").on("click", ".content-pagination a", function (e) {
      ...
      if ($(this).parents(".isotope-filter").length > 0) {
        var filter_taxonomy = $(this)
          ...
          .find(".filter-control-list")
          .attr("data-taxonomy");
        var filter_term = $(this)
          .parents(".isotope-filter")
          .find(".filter-control-item.active")
          .attr("data-value");
        if (filter_term === "all") {
          filter_term = null;
        }
      ...
      success: function (data) {
        ...
        setTimeout(function () {
          ...
          $("html, body").animate(
            {
              scrollTop:
                $(".archive-listing", ".elementor-element-" + id).offset().top -
                200,
            },
            500
          );
        ...
        setTimeout(function () {
          var $isotope = $(".archive-listing", ".elementor-element-" + id).find(
            ".isotope-trigger"
          );
